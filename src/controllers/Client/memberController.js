...
const userValidation = require('../../validation/client/memberValidation')

const storage = multer.diskStorage({    
    destination : (req, file, cb) => {
        cb(null, 'public/upload/client/member');
    },    
...
const update = async(req, res) => {
    ...
    const existingEmail = await userModel.checkEmail(data.email, id);
    if(Object.keys(existingEmail).length > 0){
        return res.status(400).json(existingEmail)
    }
    ...
    if(!user.data){
        return res.status(200).json({message: user.errors})
    }
  
    return res.status(200).json({message:'Cap nhat thanh cong',user: data});
}
